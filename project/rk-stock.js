// rk-stock.js — Supabase-backed stock. Auto-seeds on first load.
...
const STOCK_API = (() => {
  ...
  return {
    ...
    async aisles() {
      await _init();
      const a = {};
      for (const it of _cache) (a[it.aisle] = a[it.aisle] || []).push(it);
      return AISLE_ORDER.filter(aisle => a[aisle]?.length).map(aisle => ({ aisle, items: a[aisle] }));
    },
    async lowStock(threshold) {
      await _init();
      return _cache.filter(it => it.qty <= (threshold ?? it.low));
    },
    async decrement(id, n = 1) {
      const it = _cache.find(x => x.id === id);
      if (!it) return null;
      const qty = Math.max(0, it.qty - n);
      it.qty = qty;
      await _SB.from('stock_items').update({ qty }).eq('id', id);
      return it;
    },
    async adjust(id, delta, reason = '') {
      ...
      it.qty = qty;
      it._lastReason = reason;
      await _SB.from('stock_items').update({ qty, last_reason: reason }).eq('id', id);
      return it;
    },
    async add(partial) {
      await _init();
      const id = 'sk' + (900 + _cache.length);
      const image = partial.image || (partial.ean ? offImageUrl(partial.ean) : null);
      const row = { id, qty: 0, low: 6, aisle: 'Pantry', ...partial, image };
      await _SB.from('stock_items').insert(row);
      _cache.push(_withImage(row));
      return _withImage(row);
    },
    async scan(ean) {
      try {
    ...
    async suggestSub(itemId) {
      ...
      if (!it) return null;
      const peers = _cache.filter(x => x.aisle === it.aisle && x.id !== it.id && x.qty > 0);
